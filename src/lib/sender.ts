...
export const sender = ({ token, vkToken, chatId, criedHalfBea }) => {
  const telegram = new Telegram(token)
  const vkapi = vkApi(vkToken)
  return {
    async sendDocument(document: string, text = '', extra = defaultConfig) {
      const messages = []
      let doc = (
        await vkapi.docs.getById(`${document.owner_id}_${document.id}`)
      ).response[0]
      if (!doc) doc = document
      if (
        doc.ext === 'gif' &&
        doc.preview &&
        doc.preview.video &&
        doc.preview.video.file_size < 50000000
      ) {
        ...
        if (!text) {
          messages.push(
            await telegram.sendAnimation(chatId, doc.preview.video.src, extra)
          )
    ...
    async sendPhoto(photo, text = '', extra = defaultConfig()) {
      if (!text) {
        return telegram.sendPhoto(
          chatId,
          utils.photo.getHQ(photo, criedHalfBea),
          extra
        )
    ...
    async sendVideo(video, text = null, extra = defaultConfig()) {
      const apivideo = (await vkapi.video.get(`${video.owner_id}_${video.id}`))
        .response.items[0]
      if (apivideo) {
      ...
      return telegram.sendMessage(
        chatId,
        `<a href="${
          video.player.match(/^https:\/\/vk\.com\/video/i)
            ? utils.video.getPreview(video)
            : video.player
        }">\u{E0020}</a>${
          text ? `${text}\n\n` : ''
        }<a href="https://vk.com/video${video.owner_id}_${video.id}">${
          video.title ? video.title : `video${video.owner_id}_${video.id}`
        }</a>`,
        extra
      )
    }
