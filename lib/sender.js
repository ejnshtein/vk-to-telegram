    ...
    return {
        ...
        async sendVideo (video, text = null, extra = defaultConfig()) {
            ...
            return telegram.sendMessage(chatId,
                `<a href="${video.player.match(/^https:\/\/vk\.com\/video/i) ? utils.video.getPreview(video) : video.player}">&#160;</a>${text ? `${text}\n\n` : ''}<a href="https://vk.com/video${video.owner_id}_${video.id}">${video.title ? video.title : `video${video.owner_id}_${video.id}`}</a>`,
                extra)
        }
    }
}