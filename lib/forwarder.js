...
module.exports = (config) => {
  ...
  return async post => {
    ...
    mediaText = mediaText
      ...
      .replace(/>/gi, '&gt;')
      .replace(/&/gi, '&amp;')
      .replace(/\[(\S+)\|([\S\s]*?)\]/ig, '<a href="https://vk.com/$1">$2</a>') // parse vk markdown to html [vkId|Text] => <a href=vk.com/vkId>Text</a>
                ...
                messages.push(
                  await telegram.sendPoll(
                    ...
                    config.customPollTitle
                      ? `${config.customPollTitle} - ${decode(attachments[i].poll.question)}`
                      : decode(attachments[i].poll.question),
                    attachments[i].poll.answers.map(answer => answer.text),
                    {
                      reply_markup: mediaExtra.reply_markup ? mediaExtra.reply_markup : {}
                    }
                  )
                )
              }
